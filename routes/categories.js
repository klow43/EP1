...
router.get('/', async function(req, res, next) {
    ...
    }catch(err){ console.log(err); res.status(500).json({ status : "error", statusCode : 500, data : { result : "Cannot retrieve categories."}})}

    categories[0] == null ? 
      res.status(500).json({ status : "error", statusCode : 500, data : { result : "No categories found.", products : categories } }) :
...
router.get('/:categoryid', async function(req, res, next){
    ...
    }catch(err) { console.log(err); res.status(500).json({ status : "error", statusCode : 500, data : { result : "Cannot retrieve category."}})};

    category == null ?
        res.status(500).json({ status : "error", statusCode : 500, data : { result : "No category found.", products : category } }) :
            res.status(200).json({ status : "success", statusCode : 200, data : { result : "Category found.", products : category }})
})

router.post('/', isAdmin, async function(req, res, next) {
    let result;
    if(!req.body.category){res.status(400).json({ status : "error", statusCode : 400, data : { result : "category must be provided." }})}
    try{
       result = await categoryService.createCategory(req.body.category); 
    }catch(err){console.log(err); res.status(500).json({ status : "error", statusCode : 500, data : { result : "Unable to create category."}})}
...
router.put('/', isAdmin, async function(req, res, next) {
    ...
    try{
        await categoryService.alterCategory(req.body.id, req.body.category);
    }catch(err){ console.log(err); res.status(500).json({ status : "error", statusCode : 500, data : { result : "Cannot alter category."}})}
...
router.delete('/', isAdmin, async function(req, res, next) {
    ...
    }catch(err){ console.log(err); res.status(500).json({ status : "error", statusCode : 500, data : { result : "Unable to delete category" }})}
